...
- **Inquiries** (quote / sample / contact) are one unified `Inquiry` model with a `type` discriminator; quote line items live in `InquiryItem`; sales reps' per-quote notes live in `InquiryNote`. All inbound messages land in `/admin/requests` (which also shows rep notes read-only).
...
- **Portal auth (customers + sales reps):** DB-backed **better-auth** (email/password, `disableSignUp`). Both portals gate in-layout (not `proxy.ts`), keyed by `User.role`: customers (`role="customer"`) use `/portal`; sales reps (`role="rep"`, one-to-one to their `SalesRep` record via `SalesRep.userId`) use `/sales`. `getSalesSession()` (`lib/sales.ts`) gates `/sales` on role + an `active` linked rep, and every rep read/mutation is ownership-scoped to that rep's customers/quotes. Cross-role visitors are redirected (`/portal`↔`/sales`).
- **Account onboarding (no admin-typed passwords):** admins provision customers/reps in the admin UI with a random password; the system emails a token-gated set-password link via better-auth's password-reset flow (`sendResetPassword` → `emails/account-invite.tsx`, 72h expiry, `lib/auth/provision.ts`). Invitees activate at `/set-password?portal=…&token=…`; `onPasswordReset` stamps `User.activatedAt` (drives the admin Pending/Active badge). Admins can "Resend invite". Provisioning is best-effort on email (succeeds with a console warning when `RESEND_API_KEY` is unset).
- **Env:** copy `.env.example` → `.env`. Required: `DATABASE_URL`, `ADMIN_PASSWORD`, `SESSION_SECRET`. Optional: `RESEND_API_KEY` (email sending is skipped with a console warning when unset), `SITE_PASSWORD` (when set, public pages require a preview password at `/preview` once per browser session — portal/sales/set-password and the catalog JSON APIs keep their own auth and stay reachable; unset = public site).
