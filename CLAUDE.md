...
- **Company:** the customer account is a `Company` (name, unique MEC account number, industry, location, sales-rep assignment via `salesRepId`), plus the paper-form fields (business type, TRN, tax exemption #, billing/shipping address, accounting contact) and admin-only account terms (sector, credit terms, credit limit, GCT status), not an individual user. Portal users belong to a company via `User.companyId` (nullable — a customer user without a company is orphaned/unlinked). Admins manage companies and their users at `/portal/customers` (numeric `Company.id` routes, e.g. `/portal/customers/[id]`). `Inquiry` rows carry a `companyId` (relation `companyRef`) used for company-scoped history and rep routing, alongside the legacy freeform `company` string.
...
- **Catalog export:** admins download the full catalog as Excel from the "Export to Excel" button on `/portal/products` (`GET /api/admin/products/export`, admin-gated). `lib/catalog-export.ts` builds it with `exceljs`: one row per SKU (products with no variants still get a row), all product + variant fields, and a 96 px PNG thumbnail of the variant image (falling back to the product image) embedded in the Image column via `sharp` — thumbnails are generated once per distinct path with bounded concurrency (≈ 20 s for ~800 SKUs). `sharp` and `exceljs` are in `serverExternalPackages`. There is no importer for this file; it is a read-only snapshot.
...
- **Guest quote funnel (SOP §D/E):** guest `submitQuote` runs `lib/customer-match.ts` (email → phone → normalised company name against portal `User`/`Company` rows) and stores `Inquiry.matchStatus` (`VERIFIED` | `POTENTIAL_MATCH` | `NO_MATCH`), an opaque `Inquiry.ref` token, guest `industry`/`location`, and an admin-only `matchedCompanyId` hint. Potential matches are pointed to `/portal/sign-in` or `/portal/recover` (company name + MEC account number → reset links to the users on file; stamps the quote as verified). Signing in from that panel lands on `/portal/claim?ref=…`, which attaches the guest's own quote (proof = the unguessable `ref` + a session) to their company; admins can open any request at `/portal/requests/[id]` and attach/detach/re-assign a quote to a company manually (`attachInquiryToCompany`). No-match guests go to `/register?ref=…` (prefilled New Customer Form mirroring MEC's paper form — business/TRN/billing+shipping address/principal/accounting contact → `CustomerApplication`), reviewed at `/portal/applications` by admins and the `ar` role. Approval is two-step: **Approve** (admin or ar) only marks the application `APPROVED` and emails admins; an admin then opens `/portal/customers/new?application=<id>` (prefilled `CompanyForm`), fills the internal fields (MEC account number, credit terms, credit limit, GCT status, sector, sales rep) and saves — `createCompany` creates the `Company`, provisions the principal (approved-copy invite with the account number), links the quote as VERIFIED, and stamps `ACCOUNT_CREATED` + `accountCreatedBy`. An approved-but-not-created application can be returned to review (`revertApplicationApproval`). Request info / Reject email the applicant; the info-request email links back to `/register?ref=…`, where the applicant edits and resubmits — the row returns to `SUBMITTED` with `resubmittedAt` stamped (shown as a "Resubmitted" tag in the Awaiting review queue and on the detail page). The form requires a **Business Registration Certificate** upload (PDF/PNG/JPG/WEBP ≤ 6 MB; optional on resubmit when one is already on file) — files are stored privately on the data volume at `data/applications/` (`lib/application-files.ts`, never under `public/`) and served only to admin/ar via `/portal/applications/[id]/certificate`; `next.config.ts` raises the Server Action body limit to 8 MB for this. Constant responses on forgot/recover; per-IP limits via `checkRateLimit("bucket:ip", …)`. The industry dropdown is `lib/industries.ts`. `Company.mecAccountNumber` is stored normalised (uppercase, no spaces/dashes).
- **Admin auth:** the entire Accounts Portal runs on one DB-backed **better-auth** system (email/password, `disableSignUp`) — there is no separate `ADMIN_PASSWORD` gate. `User.role` selects behavior: `admin`, `customer`, `rep`, or `ar` (Accounts Receivable — applications queue only; created at `/portal/admins`) (reps are additionally one-to-one to a `SalesRep` record via `SalesRep.userId`). All four roles sign in at `/portal/sign-in` and land on the same `/portal` tree; each page under `app/portal/(protected)/` gates itself by role rather than by a per-portal layout. `getSalesSession()` (`lib/sales.ts`) additionally gates rep-only reads/mutations on an `active` linked rep and scopes them to that rep's own companies/quotes. A seed script creates one bootstrap admin (`admin@example.com` / `test123`); admins manage further admin accounts at `/portal/admins`. That seeded credential is a well-known default — rotate it (or set `SEED_ADMIN_PASSWORD`) before exposing a deployment. `/portal/forgot-password` provides self-service reset for every role.
...
- **Integration API (OneChat agents):** `POST /api/customers/verify`, `POST /api/quotes`, `GET /api/quotes/[ref]` are bearer-gated by `INTEGRATION_API_KEY` (`requireIntegrationKey` in `lib/api/http.ts`; unset ⇒ 503). Logic lives in `lib/integration/` (`verifyAccount` = MEC account number alone, with an optional company name that must match when supplied — speech-to-text mangles names on voice calls; `createQuote` mirrors `submitQuote` — verified path links the company, guest path runs `matchGuest`). Quotes from these endpoints carry `[via whatsapp|voice]` in `Inquiry.message` and may have an empty `email` (confirmation email is skipped). Contract in `docs/api.md`.
