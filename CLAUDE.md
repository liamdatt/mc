...
- **Env:** copy `.env.example` → `.env`. Required: `DATABASE_URL`, `ADMIN_PASSWORD`, `SESSION_SECRET`. Optional: `RESEND_API_KEY` (email sending is skipped with a console warning when unset), `SITE_PASSWORD` (when set, all public pages require a preview password at `/preview` once per browser session; unset = public site).
