...
- **Integration API (OneChat agents):** `POST /api/customers/verify`, `POST /api/quotes`, `GET /api/quotes/[ref]` are bearer-gated by `INTEGRATION_API_KEY` (`requireIntegrationKey` in `lib/api/http.ts`; unset ⇒ 503). Logic lives in `lib/integration/` (`verifyAccount` = account number + company name, same rule as recovery; `createQuote` mirrors `submitQuote` — verified path links the company, guest path runs `matchGuest`). Quotes from these endpoints carry `[via whatsapp|voice]` in `Inquiry.message` and may have an empty `email` (confirmation email is skipped). Contract in `docs/api.md`.
